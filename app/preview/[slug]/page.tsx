...
export default function Page() {
  ...
  const copyToClipboard = () => {
    const url = `https://devlinks-app-vera.vercel.app/preview/${userId ?? slug}`;
        ...
        <div className="md:mt-[-149px] md:shadow-[0px_0px_32px_0px_#0000001A] max-w-[349px] w-full mx-auto rounded-[24px] min-h-[569px] bg-white py-[48px] px-[24px] md:px-[56px]">
          <header className="grid place-items-center mb-[42px]">
            {UserProfilePreviewData?.profile_picture ? (
              ...
              <div className="w-[96px] h-[96px] rounded-full bg-[#eeeeee] mb-[24px]"></div>
            )}
            {UserProfilePreviewData?.first_name ||
            UserProfilePreviewData?.last_name ? (
              <h2 className="text-[18px] leading-[27px] font-semibold text-gray mb-[4px]">
                {UserProfilePreviewData.first_name}{" "}
                {UserProfilePreviewData.last_name}
              </h2>
            ) : (
            ...
            {UserProfilePreviewData?.email ? (
              <a
                href={`mailto:${UserProfilePreviewData?.email}`}
                className="text-[14px] leading-[21px] text-gray-alt"
              >
                {UserProfilePreviewData?.email}
              </a>
            ) : (
