  ...
  return (
    <AppLayout>
      <div className="flex flex-col h-full">
        {user ? (
          <ProfileInformation user={user} setUser={setUser} save={save} />
        ) : (
...
const ProfileInformation = ({ user, setUser, save }: UserContextType) => {
  ...
  const profilePicsHandler = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      const file = files[0];
      try {
        const cloudinaryUrl = `https://api.cloudinary.com/v1_1/dtn6bfvnz/image/upload`;

        // Upload image to Cloudinary
        const formData = new FormData();
        formData.append("file", file);
        formData.append("upload_preset", "devlinks"); // Use the preset name "devlinks"
        formData.append("cloud_name", "dtn6bfvnz"); // Your Cloudinary cloud name
        formData.append("folder", "devlinks"); // Uploads will go into the "devlinks" folder
        ...
        // Construct the URL for the uploaded image with transformations
        const publicId = uploadResult.public_id;
        const transformedUrl = `https://res.cloudinary.com/dtn6bfvnz/image/upload/w_65,h_65/${publicId}`;
    ...
    <>
      <div className="p-[16px] md:p-[40px]">
        <header className="mb-[40px]">
          <h1 className="text-gray font-bold text-[24px] lg:text-[32px] mb-[8px]">
            Profile Details
          </h1>
        ...
        <div className="flex flex-col gap-[24px]">
          <div className="bg-whitesmoke rounded-[12px] p-[20px] flex flex-col md:flex-row gap-[16px] md:justify-between md:items-center">
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-[24px]">
              <div className="rounded-[12px] overflow-hidden w-[193px] grid place-items-center shrink-0 h-[193px] rounded-[12px] bg-primary-light">
